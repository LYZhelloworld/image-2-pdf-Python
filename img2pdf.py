...
def getImages(path):
    ...
    return [os.path.join(path, i) for i in files if i.lower().endswith(
    ...
    if len(sys.argv) > 1:
        for directory in sys.argv[1:]:
            ...
            if len(images) > 0:
                ...
                print(
                    'PDF file created: {} with {} image file(s).'.format(
                        os.path.basename(pdf), len(images)))
        input('Press any key to continue...')  # Pause
    else:
        print(
            'Usage: ' +
            sys.argv[0] +
            ' <directory that contains images> [<directory2 that contains images> ...]\nYou can drag and drop folders onto the script.')
